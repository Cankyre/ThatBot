        ...
        if (interaction.member.permissions.has("ADMINISTRATOR")) {
            ...
            try {
                ...
                if (
                    ...
                    interaction.reply(
                        ":x: Please add a role that is lower than the bot's highes role!"
                    );
                }
            } catch {
            ...
            interaction.channel
                .send(
                    "Last step: react to this message with the emoji you want people to react with. (in the next 30 seconds)"
                )
                .then((sent) => {
                    sent.awaitReactions({
                        max: 1,
                        ...
                        try {
                            ...
                            RRData.set(
                                message_id + emoji.name + emoji.id,
                                role.id
                            );
                            sent.reactions
                                .removeAll()
                                .then(sent.edit(":white_check_mark: All set!"));
                            message.react(emoji);
                        } catch (err) {}
                    });
                });
